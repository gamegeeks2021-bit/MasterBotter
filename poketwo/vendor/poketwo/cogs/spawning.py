...
class Spawning(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Mark text activity for dynamic spawn rate.
        controller = getattr(self.bot, "poketwo_activity", None)
        if controller is not None:
            try:
                controller.note_text(message.guild.id)
            except Exception:
                pass
        ...
        # Dynamic spawn rate: if the server is active (recent text activity or voice activity),
        # increase spawn rate by lowering the threshold. When inactive, stay at the base rate.
        base_threshold = 8 if message.guild.id == 716390832034414685 else 24
        controller = getattr(self.bot, "poketwo_activity", None)
        if controller is not None:
            try:
                spawn_threshold = int(controller.spawn_threshold(message.guild.id, base_threshold))
            except Exception:
                spawn_threshold = base_threshold
        else:
            spawn_threshold = base_threshold
    ...
    async def catch(self, ctx, *, guess: str):
        ...
        message = ctx._(
            "caught", species=str(species), trainer=ctx.author.mention, level=level, ivPercentage=sum(ivs) / 186 * 100
        )
    ...
    def cog_unload(self):
        # In some embed-integrations, these tasks may not exist if the cog
        # was loaded/unloaded before startup completed.
        if hasattr(self, "spawn_incense"):
            self.spawn_incense.cancel()
        if hasattr(self, "send_spawns"):
